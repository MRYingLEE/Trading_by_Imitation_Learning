...
def create_network_given_future(nb_classes, winner, weight_path='weights/optimal_3_stocks.h5'):
    model = Sequential()
    ...
    try:
        if winner:
            ...
            model.load_weights(weight_path+"-loser.h5")
...
def train_optimal_action_given_future_obs(model, target_history, target_stocks, winner,
                                          weight_path='weights/optimal_3_stocks.h5'):
    (X_train, y_train), (X_test, y_test) = create_optimal_imitation_dataset(target_history, winner)
    ...
    while continue_train:
        ...
        if save_weights:
            if winner:
                ...
                model.save(weight_path+"-loser.h5")

        continue_train = input('True to continue train, otherwise stop\n')


def create_network_give_past(nb_classes, window_length, winner, weight_path='weights/imitation_3_stocks.h5'):
    model = Sequential()
...
def train_optimal_action_given_history_obs(model, target_history, target_stocks, window_length, winner,
                                           weight_path='weights/imitation_3_stocks.h5'):
    nb_classes = len(target_stocks) + 1
    (X_train, y_train), (X_validation, y_validation) = create_imitation_dataset(target_history, window_length,winner=True)
    Y_train = np_utils.to_categorical(y_train, nb_classes)
