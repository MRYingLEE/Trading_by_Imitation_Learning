...
def create_optimal_imitation_dataset(history, winner, training_data_ratio=0.8, is_normalize=True):
    ...
    if winner:
        ...
        labels = np.argmin(close_open_ratio, axis=1)
...
def create_imitation_dataset(history, window_length, winner, training_data_ratio=0.8, is_normalize=True):
    ...
    for i in range(window_length, T):
        ...
        if winner:
            ...
            label = np.argmin(close_open_ratio[:, i:i+1], axis=0)
